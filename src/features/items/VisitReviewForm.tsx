...
export function VisitReviewForm({ placeId, visit, review, onClose }: { placeId: number; visit: PlaceVisit; review?: PlaceVisitReview; onClose: () => void }) {
  ...
  const mutation = useMutation({ mutationFn: (form: FormData) => { const input: PlaceVisitReviewInput = { overall, comment: String(form.get("comment")).trim() || undefined, taste, price }; return review ? updateVisitReview(review.id, input) : createVisitReview(visit.id, input); }, onSuccess: async () => { await invalidate(); showNotice(review ? "Actualizamos la reseña compartida." : "Agregamos la reseña a esta visita."); onClose(); } });
  ...
  return <Modal onClose={onClose} confirmDiscard pending={mutation.isPending || remove.isPending}><form onSubmit={(event) => { event.preventDefault(); mutation.mutate(new FormData(event.currentTarget)); }}><p className="eyebrow">RESEÑA DE LA VISITA</p><h2>¿Cómo estuvo?</h2><p className="muted">La puntuación general es obligatoria. Sabor y precio son opcionales.</p>{score("Puntuación general", overall, (value) => { if (value !== undefined) setOverall(value); })}<div className="score-grid">{score("Sabor", taste, setTaste, true)}{score("Precio", price, setPrice, true)}</div><label>Comentario <small className="tiny">Opcional</small><textarea className="review-textarea" name="comment" defaultValue={review?.comment} maxLength={2000} placeholder="Contá la experiencia…" /></label><Button icon={review ? "💾" : "💬"} disabled={mutation.isPending || remove.isPending}>{mutation.isPending ? "Guardando…" : review ? "Guardar reseña" : "Agregar reseña"}</Button>{review && <Button variant="destructive" icon="🗑️" type="button" disabled={mutation.isPending || remove.isPending} onClick={() => remove.mutate()}>Borrar reseña</Button>}{(mutation.error || remove.error) && <p className="form-error">{(mutation.error || remove.error)!.message}</p>}</form></Modal>;
}
