...
export function ActivityReviewForm({ activityId, visit, review, onClose }: { activityId: number; visit: ActivityVisit; review?: ActivityReview; onClose: () => void }) {
  ...
  return <Modal onClose={onClose} confirmDiscard pending={mutation.isPending || remove.isPending}><form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}><p className="eyebrow">RESEÑA DE LA SALIDA</p><h2>¿Cómo la pasaron?</h2><label>Puntuación<StarRating label="Puntuación de la salida" value={rating} onChange={setRating} /></label><label>Comentario <small className="tiny">Opcional</small><textarea className="review-textarea" value={comment} maxLength={1000} onChange={(event) => setComment(event.target.value)} placeholder="Contá la experiencia…" /></label><Button icon={review ? "💾" : "💬"} disabled={mutation.isPending || remove.isPending}>{mutation.isPending ? "Guardando…" : review ? "Guardar reseña" : "Agregar reseña"}</Button>{review && <Button variant="destructive" icon="🗑️" type="button" onClick={() => remove.mutate()}>Borrar reseña</Button>}{(mutation.error || remove.error) && <p className="form-error">{(mutation.error || remove.error)!.message}</p>}</form></Modal>;
}
