...
export function FilmForm({ onClose, film }: { onClose: () => void; film?: Film }) {
  ...
  const save = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      try {
        ...
        return { film: saved, photoError: error instanceof Error ? error.message : 'No pudimos subir el póster' };
      }
    },
    onSuccess: async result => {
      ...
      showNotice(result.photoError ? `Guardamos los cambios, pero el póster no se subió: ${result.photoError}` : 'Actualizamos la película.', result.photoError ? 'error' : 'success');
      onClose();
    },
  });
  ...
  return <Modal onClose={onClose} confirmDiscard pending={save.isPending}><form className="film-form" onSubmit={event => { event.preventDefault(); save.mutate(new FormData(event.currentTarget)); }}>
    ...
    {isTmdbFilm || manualMode || film ? <><div className="form-columns"><label>Plataforma<select name="platformId" defaultValue={film?.platform?.id ?? ''}><option value="">Todavía no sabemos</option>{availablePlatforms.map(platform => <option key={platform.id} value={platform.id}>{platform.icon} {platform.name}{!platform.active ? ' (inactiva)' : ''}</option>)}</select></label></div><button className="main-button" disabled={save.isPending || Boolean(photoError)}>{save.isPending ? 'Guardando…' : film ? 'Guardar película' : 'Guardar en WhichFilm'} ✦</button>{save.error && <p className="form-error">{save.error.message}</p>}</> : null}
  </form></Modal>;
}
