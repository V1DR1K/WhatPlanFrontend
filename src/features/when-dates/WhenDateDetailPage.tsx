...
export function WhenDateDetailPage() {
  ...
  return <section className="when-date-detail">
    ...
    <section className="when-date-source-photos"><div className="section-title"><div><p className="eyebrow">RECOPILADO</p><h2>Fotos de las experiencias</h2></div><strong>{sourcePhotos.length}</strong></div>{sourcePhotos.length ? <div className="when-date-source-photos__grid">{sourcePhotos.map((photo) => <a key={photo.id} href={mediaUrl(photo.url)} target="_blank" rel="noreferrer" aria-label={`Abrir foto de ${photo.title}`}><img src={mediaUrl(photo.thumbnailUrl || photo.url)} alt={`Foto de ${photo.title}`} loading="lazy" decoding="async" /><span>{photo.title}</span></a>)}</div> : <p className="empty-state">Las experiencias recopiladas todavía no tienen fotos.</p>}</section>
