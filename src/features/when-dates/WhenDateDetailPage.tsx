...
export function WhenDateDetailPage() {
  ...
  const value = detail.data!; const ownComment = value.comments.find((comment) => comment.author === session.get()?.username);
  return <section className="when-date-detail">
    ...
    <ExperienceGallery accentLabel="RECUERDOS" emptyIcon="💝" name={value.specialDate.label} photos={value.photos} coverPhotoId={value.coverPhoto?.id} onUpload={(files) => uploadPhotos.mutateAsync(files)} onSetCover={(photo) => cover.mutate(photo.id)} onDelete={setDeletingPhoto} />
