...
export function CookingReviewForm({ cooking, review, onClose }: { cooking: Cooking; review?: CookingReview; onClose: () => void }) {
  ...
  const mutation = useMutation({ mutationFn: () => review ? updateCookingReview(review.id, { rating, complexity, comment: comment.trim() || undefined }) : createCookingReview(cooking.id, { rating, complexity, comment: comment.trim() || undefined }), onSuccess: async () => { await invalidate(); showNotice(review ? "Actualizamos la reseña compartida." : "Agregamos la reseña a esta cocinada."); onClose(); } });
  ...
  return <Modal onClose={onClose} confirmDiscard pending={mutation.isPending || remove.isPending}><form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}><p className="eyebrow">RESEÑA DE LA COCINADA</p><h2>¿Cómo salió?</h2><label>Puntuación<StarRating label="Puntuación de la cocinada" value={rating} onChange={setRating} /></label><label>Complejidad<StarRating label="Complejidad de la receta" value={complexity} onChange={setComplexity} /></label><label>Comentario <small className="tiny">Opcional</small><textarea className="review-textarea" value={comment} maxLength={1000} onChange={(event) => setComment(event.target.value)} placeholder="Contá qué gustó o cambiarías…" /></label><Button icon={review ? "💾" : "💬"} disabled={mutation.isPending || remove.isPending}>{mutation.isPending ? "Guardando…" : review ? "Guardar reseña" : "Agregar reseña"}</Button>{review && <Button variant="destructive" icon="🗑️" type="button" onClick={() => remove.mutate()}>Borrar reseña</Button>}{(mutation.error || remove.error) && <p className="form-error">{(mutation.error || remove.error)!.message}</p>}</form></Modal>;
}
