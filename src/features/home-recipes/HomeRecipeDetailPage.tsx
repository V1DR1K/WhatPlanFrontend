  ...
  return <section className="home-recipe-detail">
    ...
    <div className="home-recipe-detail__head">
      ...
      <div className="home-recipe-detail__summary">
        ...
        <h1>{recipe.name}</h1>
        <div className="home-recipe-detail__author"><span>{recipe.author[0]?.toUpperCase()}</span><p><strong>Preparó {recipe.author}</strong><small>{dateLabel(recipe.preparedOn)} · Casa de {homeName(recipe.home)} · {recipe.servings} porciones</small></p></div>
        {recipe.repeatedFrom && <p className="muted">↻ Esta preparación repite <Link to={`/how-cook/${recipe.repeatedFrom.id}`}>{recipe.repeatedFrom.name}</Link>.</p>}
      </div>
      <div className="detail-actions home-recipe-detail__actions">
        {ownRecipe && <button className="secondary-button" type="button" onClick={() => setEditing(true)}><span aria-hidden="true">✎</span> Editar receta</button>}
        <button className="secondary-button" type="button" onClick={() => setRepeating(true)}><span aria-hidden="true">↻</span> Repetir receta</button>
        {ownRecipe && <button className="text-button home-recipe-delete" type="button" disabled={remove.isPending} onClick={() => setConfirmingDelete(true)}>{remove.isPending ? 'Borrando…' : 'Borrar receta'}</button>}
      </div>
    </div>
    <section className="home-recipe-detail__content">
      <div className="home-recipe-detail__panel"><p className="eyebrow">INGREDIENTES</p><h2>Para {recipe.servings} porciones</h2><ul>{recipe.ingredients.map((ingredient, index) => <li key={`${ingredient.name}-${index}`}><strong>{ingredientLabel(ingredient)}</strong></li>)}</ul></div>
      <div className="home-recipe-detail__panel"><p className="eyebrow">RECETA</p><h2>Cómo se hace</h2><ol className="recipe-steps">{recipe.steps.map((step, index) => <li key={`${step.instruction}-${index}`}>{step.instruction}</li>)}</ol>{recipe.recipeUrl && <a className="secondary-button recipe-detail-button" href={recipe.recipeUrl} target="_blank" rel="noreferrer">Ver fuente original ↗</a>}</div>
    </section>
    {recipe.repetitions.length > 0 && <section className="home-recipe-repetitions"><p className="eyebrow">HISTORIAL</p><h2>La volvieron a preparar</h2><div>{recipe.repetitions.map(repetition => <Link key={repetition.id} to={`/how-cook/${repetition.id}`}>↻ {repetition.name}</Link>)}</div></section>}
    <section className="reviews-section home-recipe-reviews"><div className="section-title"><div><p className="eyebrow">RESEÑAS DE ESTA PREPARACIÓN</p><h2>Tomás y Avril</h2></div><strong>{recipe.reviews.length}/2</strong></div><div className="home-recipe-review-columns">{reviewers.map(author => <ReviewCard key={author} author={author} currentUser={username} review={recipe.reviews.find(value => value.author === author)} onReview={() => setReviewing(true)} />)}</div></section>
    {confirmingDelete && <ConfirmDialog title="¿Borrar esta receta?" message={remove.error ? remove.error.message : 'La receta y su foto dejarán de estar disponibles en la casa.'} confirmLabel="Borrar receta" pending={remove.isPending} onClose={() => setConfirmingDelete(false)} onConfirm={() => remove.mutate()} />}
